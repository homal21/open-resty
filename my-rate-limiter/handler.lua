...
function CustomRateLimiter:access(conf)
    ...
    if is_ip_banned(red, ban_key) then
        ...
        kong.response.exit(403, { message = "IP blocked !!! Retry after " .. ban_ttl .. " s" })
    end
    ...
    if exceeded then
        ...
        if violations >= conf.max_violations then
            ...
            kong.response.exit(429,
                { message = "API limit. Violate " .. violations .. " in " .. conf.max_violations .. " will be blocked." })
        end
    else
